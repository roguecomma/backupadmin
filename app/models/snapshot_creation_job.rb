class SnapshotCreationJob < Struct.new(:frequency_bucket)
  ...
  def run(server)
    puts("SnapshotCreationJob-> server=#{server.elastic_ip}, frequency_bucket="+ (self.frequency_bucket.to_s) +", time="+ (Time.now.to_s))
    if server.is_highest_frequency_bucket?(frequency_bucket)
      puts("SnapshotCreationJob-> server=#{server.elastic_ip}, "+ (self.frequency_bucket.to_s) +" new backup requested")
      ...
      puts("SnapshotCreationJob-> server=#{server.elastic_ip}, "+ (self.frequency_bucket.to_s) +" renaming a backup")
