class BackupJob < Struct.new(:server_id, :tag)
  ...
  def remove_unneeded_backups
    # find backups for the tag for the server_id in order of age
    # if the number is greater than server.#tag then remove the tag from the oldest
    #  only remove the volume if there are no other tags associated with that backup
  end
